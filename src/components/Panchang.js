...
export default function PanchangCard() {
  ...
  useEffect(() => {
    const fetchPanchang = async () => {
      try {
        ...
        console.error(err);
        setError("Could not load Panchang. Check backend.");
      }
    };
    fetchPanchang();
  }, []);
  ...
  return (
    <div className="card">
      ...
      <p className="card-subtitle">Live data from your backend.</p>

      {error && <p style={{ color: "#ff7676", fontSize: "0.85rem" }}>{error}</p>}
